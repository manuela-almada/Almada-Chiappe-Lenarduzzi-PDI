...
#PROBLEMA 1
#--------------
def ecualizacion_local(img,v):
    alto, ancho = img.shape

    # Agregar borde reflectante para manejar bordes
    borde = v // 2
    img_borde = cv2.copyMakeBorder(img, borde, borde, borde, borde, cv2.BORDER_REFLECT)

    # Crear imagen de salida vacía
    img_heq = np.zeros_like(img)

    for y in range(alto):
        for x in range(ancho):
            y1, y2, x1, x2 = y,y + v, x, x + v

            ventana = img_borde[y1:y2, x1:x2]

            # Ecualizar histograma localmente
            ventana_eq = cv2.equalizeHist(ventana)

            # Asignar el valor del píxel central de la ventana ecualizada
            img_heq[y, x] = ventana_eq[borde, borde]
...
ecualizacion_local(img,5)

ecualizacion_local(img,20)
...
resumen=cv2.imread("resumen_final.jpg")
resumen = cv2.cvtColor(resumen, cv2.COLOR_GRAY2BGR)
plt.imshow(resumen,cmap='gray'), plt.show()
#----------------
# PROBLEMA 2 - D
#----------------
